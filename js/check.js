...
// 页面切换到 控制台操作
        ...
        $(document).off('click',' .cuur_id_cla');
        $(document).on( 'click' ,' .cuur_id_cla',function( event ){

            //console.log($(this)[0].id)
            //$(this)[0].addEventListener('click',function(){
            //    alert($(this)[0].id)
            //})
            for(var i in topo_allElement_data){
                //console.log(topo_allElement_data[i]);

                for(var r in topo_allElement_data[i]){

                    topo_allElement_data[i][r].selected=false;

                    if($(this)[0].id==r){
                        topo_allElement_data[i][r].selected=true;
                        currentNode=topo_allElement_data[i][r];

                        //topo_allElement_data[i][$(this)[0].id].addEventListener('click',function(){
                        //    alert(111)
                        //})
                    }
                }
...
// 命令行 键盘事件
        $(document).off( 'keydown' ,'#'+curr_id+' .cmd_input');
        $(document).on( 'keydown' ,'#'+curr_id+' .cmd_input',function( event ){
            ...
            if( event.keyCode == 13 ){
                ...
                //console.log(curr_id);
                //console.log(current_element().element_id)
...
// 汇车等待提示符时
        $(document).off( 'keypress' ,'#'+curr_id+' .cmd_wait')
        $(document).on( 'keypress' ,'#'+curr_id+' .cmd_wait',function( event ){
            if( event.keyCode == 13 ){
                var obj = $('#'+curr_id +' .cmd_box');
                //var obj = $(this).parents('.cmd_box');
                $(this).parent().html('　');
...
// 关闭页面弹窗 停止后续的请求
       $(document).off('click','.con_button');
        $(document).on('click','.com_button',function(){
            ...
            //$(document).off('click','.go_con');
            //$(document).off( 'click' ,' .cuur_id_cla');
            //$(document).off( 'keydown' , '.cmd_input')
...
// 命令对象
        var ConsoleCommand = {
            ...
            exec_ctrl_c:function( command , obj , on ){
                // 命令开启状态
                ...
                $.getJSON(window_topo_web_path+'/topo_manage/topo_manage/exec_cmd?callback=?',data,function(msg){
                    //console.log(data.topo_name_id);
                    //console.log(data.dev_name)
                    //console.log(data.cmd_id)
                    //console.log(data.cmd)
                    //console.log(data.cmd_status)
                    //console.log(msg)
                }).error(function(){
                    // 移除等待提示符
        ...
        var HotKeyHandler={
            ...
            Init:function(){
                HotKeyHandler.Register(0,"C",function( obj ){
                    if(curr_id===current_element().element_id){
                        console.log('ctrl+c');
                        console.log(curr_id)
                        ConsoleCommand.exec_ctrl_c()
            ...
            Register:function(tag,value,func){
                ...
                $(document).off('keyup','.cmd_wait, .cmd_input')
                $(document).on('keyup','.cmd_wait, .cmd_input',function(event){
                    HotKeyHandler.currentMainKey=null;
                });
                $(document).off('keydown','.cmd_wait, .cmd_input')
