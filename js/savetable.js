...
$(".keep-successed").click(function(){
    alert_func.success( '保存成功' , 1200 )
})
...
$("#new_vlan_btn_sure2_dns").click(function(){
    ...
    str += '<td style="width: 50px;" >'+domain+'.testdomain.com'+'</td>';
...
// 2层交换 添加 VLAN 页面展示
$('#modal-switchboard-form-2-btn2,#modal-switchboard-form-2-btn').click(function () {
        var element = current_element();
...
$("#new_vlan_btn_sure2,#new_vlan_btn_sure").click(function(){
    ...
    restore.switch_vlan_show( element.type );
})
...
$("#modal-switchboard-form-4-btn2,#modal-switchboard-form-4-btn").click(function(){
    ...
    if( check_obj.length == 0 ){
        alert_func.error( '请选择' , 1200);return;
    }
    check_obj.each(function(){
        ...
        if(
            ...
            if( JSON.stringify( element['netdata']['vlan_list']['interface_id_ed'][vlan_id]) != '{}' ){
                alert_func.error( '存在成员端口，不可删除' , 1200)
                return;
            }
        }
...
$("#modal-switchboard-form-6-btn2,#modal-switchboard-form-6-btn").click(function(){
    ...
    if( element['netdata'] && element['netdata']['vlan_list'] && element['netdata']['vlan_list']['data'] ){
        ...
        for( var i in element['netdata']['vlan_list']['data']){
            var val = element['netdata']['vlan_list']['data'][i]['vlan_id'];
            str+='<span style="display: inline-block;width: 70px"><input class="'+form['vlan_name'][key]+'" type="checkbox" name="'+form['vlan_name'][key]+'" value="'+i+'">'+val+'</span>'
        }

        $("#"+form['vlan_select'][key]).html(str)

        if( data['vlan_id_all'] ){
            for ( var vid in data['vlan_id_all'] ){
                $("[name='"+form['vlan_name'][key]+"'][value='"+vid+"']").prop('checked',true)
            }
        }
    }
...
$(document).on('change','#port_link_type2',function(){
    if( $(".vlan_id_2").length > 0 ){
        $(".vlan_id_2").prop('checked',false)
    }
})

$(document).on('click','.vlan_id_2',function(){
    var type = $("#port_link_type2").val();
    var ids = $(".vlan_id_2:checked").length;
    if( type != 'trunk' ){
        if( ids >= 2 ){
            ...
            $(this).prop('checked',false)
            alert_func.error( '链接类型为none无法选择' , 2000)
        }
    }
})
...
$(document).on('click','.m_mirror_port',function(){
    ...
    var ids = $(".m_mirror_port:checked").length;

        if( ids >= 2 ){
            $(this).prop('checked',false);
            alert_func.error( 'mirror port只能选择一个' , 2000)
        }


})
$(document).on('change','#port_link_type',function(){
    if( $(".vlan_id_1").length > 0 ){
        $(".vlan_id_1").prop('checked',false)
    }
})

$(document).on('click','.vlan_id_1',function(){
    var type = $("#port_link_type").val();
    var ids = $(".vlan_id_1:checked").length;
    if( type != 'trunk' ){
        if( ids >= 2 ){
            $(this).prop('checked',false)
        }
...
$("#new_port_btn_sure2,#new_port_btn_sure").click(function(){
    ...
    var co = 'down';
    if( $("[name='"+form['co'][key]+"']:checked").val() == 'true' ){
         co = 'up';
    }
    var saveobj = element['netdata']['interface_list']['data'][element.interface_edit_id]

    var old_vlan_id = saveobj.vlan_id_all;
    // 删除之前的 vlanid 关系
    if( old_vlan_id && JSON.stringify(old_vlan_id) != '{}' ){
        for( var ovid in old_vlan_id ){
            delete element['netdata']['vlan_list']['interface_id_ed'][ovid][element.interface_edit_id]
        }
    }
    ...
    if( saveobj.vlan_id_all ){
        old_vlan_id = saveobj.vlan_id_all;
    }
    saveobj.vlan_id_all = {};

    if( $("[name='"+form['vlan_name'][key]+"']:checked").length > 0 ){
        $("[name='"+form['vlan_name'][key]+"']:checked").each(function(){
            saveobj.vlan_id_all[$(this).val()] = $(this).val();
        })
    }
    ...
    if( JSON.stringify(saveobj.vlan_id_all) != '{}' ){
        var vidstr='';
        for( var vid in saveobj.vlan_id_all ){
            if( element['netdata']['vlan_list'] && element['netdata']['vlan_list']['data'][vid] ) {

                if (!element['netdata']['vlan_list']['interface_id_ed']) {
                    element['netdata']['vlan_list']['interface_id_ed'] = {};
                }

                if (!element['netdata']['vlan_list']['interface_id_ed'][vid]) {
                    element['netdata']['vlan_list']['interface_id_ed'][vid] = {};
                }

                element['netdata']['vlan_list']['interface_id_ed'][vid][element.interface_edit_id] = 1;

                vidstr += element['netdata']['vlan_list']['data'][vid]['vlan_id']+"/"
            }
        }
        if( vidstr ){
            vidstr = vidstr.substr(0,vidstr.length-1)
        }
        str += '<td style="width: 50px;" >' + vidstr + '</td>';
    }else{
        str += '<td style="width: 50px;" ></td>';
    }
    ...
    if( element['netdata']['vlan_list'] && element['netdata']['vlan_list']['interface_id_ed'] ){
        ...
        for( var vid in old_vlan_id ){
            var text = '';
            for( interface_name in element['netdata']['vlan_list']['interface_id_ed'][vid] ){
                text += interface_name + '/'
            }
            if( text ){
                text = text.substr(0,text.length-1)
            }
            $('[name="'+element.vlan_checkbox_name+'"][value="'+vid+'"]').parents('tr').find('td:last').text( text )
        }
...
$("#new_mi_btn_sure2,#new_mi_btn_sure").click(function(){
    ...
    if( mirror_id == '0' ){
        alert_func.error( '请选择镜像组ID' , 1200)
        return;
    }
    if( monitor_port.length == 0 ){
        alert_func.error( '请选择 monitor port' , 1200)
        return;
    }
    if( mirror_port.length == 0 ){
        alert_func.error( '请选择 mirror port' , 1200)
        return;
    }
    ...
    if( show_monitor_port ){
        show_monitor_port = show_monitor_port.substr(0,show_monitor_port.length-1)
    }
    ...
    str += '<td style="width: 110px;word-wrap: break-word;">'+show_monitor_port+','+'</td>';
...
$('#new_rip_btn,#new_OSPF_btn,#new_bgp_btn').click(function () {
    ...
    if( $(this).attr('id') == 'new_bgp_btn' ){
        $('#new_router_rip>div>p').html('新建bgp路由');
        $('#area_gg').hide();
        $('#new_router_rip_table_f_td').html('网段/掩码')
    }else if( $(this).attr('id') == 'new_OSPF_btn' ){
        $('#new_router_rip>div>p').html('新建OSPF路由');
        $('#area_gg').show();
        $('#new_router_rip_table_f_td').html('网段/掩码/区域')
    ...
    if( element['netdata']['route_dynamic']
        ...
        for( i in element['netdata']['route_dynamic'][dynamic_type]['data'] ){
            if(element['netdata']['route_dynamic'][dynamic_type]['data'][i].area){
                str +="<tr id='"+dynamic_type+"-"+i+"'>";
                str += '<td >'+i+'</td>';
                str += '<td >'+element['netdata']['route_dynamic'][dynamic_type]['data'][i].net_duan+'/'+element['netdata']['route_dynamic'][dynamic_type]['data'][i].mask+'/'+element['netdata']['route_dynamic'][dynamic_type]['data'][i].area+'</td>';
...
$("#add_route_route").click(function(){
    ...
    var area =$('#OSPF_zdwh_area').val();
    if( !alertMsg(net_duan,'内网网段不能为空') ){
        return ;
    }
    if( !alertMsg(mask,'掩码不能为空') ){
        return ;
    }
...
$("#new_router_port_btn_sure").click(function(){
    var element = current_element();
    var co = 'down';
    if( $("[name='new_router_port_kg']:checked").val() == 'true' ){
        co = 'up';
    }
...
$("#firewall_modal2_s").click(function(){
    var element = current_element();
    var co = 'down';
    if( $("[name='firewall_kg']:checked").val() == 'true' ){
        co = 'up';
    }
...
$('#modal-firewall-form-6-btn').click(//编辑
    function () {
        ...
        if( element.netdata['firewall_safe']['data'][check_id].status == 'true' ){
            $("#port1_saf").prop('checked',true)
            $("#port2_saf").prop('checked',false)

        }else{
            $("#port2_saf").prop('checked',true);
            $("#port1_saf").prop('checked',false)
        ...
        // 源端口范围起点
        $('#new_firewall_saf_yuan_rate_end').val(element.netdata['firewall_safe']['data'][check_id].source_port_end );
        // 目的地址
...
$("#new_firewall_saf_btn_sure").click(function(){
    ...
    var table_data = {
        ...
        new_firewall_saf_yuan_rate_end:{
            msg:'源端口范围不能为空',
            key:'source_port_end',
        },
...
$("#sdn_new_port_btn_sure2").click(function(){
    var element = current_element();
    var co = 'down';
    if( $("[name='sdn_port_kg2']:checked").val() == 'true' ){
        co = 'up';
    }
...
// ==================================================
$("#modal-sdnC-form-2-btn-getIP").click(function(){
    var topo_id = $("#current_topo_id").val();
    var data = {
        'topo_name_id':topo_id
    }
    Ajax$.jsonp({
        url:window_topo_web_path+'/topo_manage/topo_manage/get_sdn_controller_port?callback=?',
        data:data,
        ok:function(msg){
            if(  msg && msg['retCode'] != undefined && msg['retCode'] == 0  ){

                $("#sdnC-2").val( msg.ipaddr )
                $("#sdnC-3").val( msg.port )
            }else{
                alert_func.error( '获取失败' , 1200)
            }
        },
        error:function(){
            alert_func.error( '获取失败' , 1200)
...
function editlist( name ){
    ...
    if( check_obj.length == 0 ){
        alert_func.error( '请选择' , 1200)
        return false;
    }
    if( check_obj.length > 1 ){
        alert_func.error( '一次只能编辑一条记录' , 1200)
        return false;
    }
    return check_obj;
}
...
var restore = {
    ...
    server_dns:function() {
        ...
        for( var i in element.netdata['dnslist'].data ){
            ...
            str += '<td style="width: 50px;" >'+element.netdata['dnslist'].data[i].domain+'.testdomain.com'+'</td>';
            str += '</tr>';
            table_obj.find('tbody').append( str )
        }
    },
    ...
    switch_restore:function(data){
        ...
        Ajax$.jsonp({
            url:window_topo_web_path+'/topo_manage/topo_manage/produce_switch_port_info?callback=?',
            data:data,
            ok:function(data){
                if( data && data.retCode != undefined && data.retCode == 0 && data.eth_list ){
    ...
    switch_2layer_html:function(data){
        ...
        for(var i=0;i<data.length;i++){
            ...
            if( JSON.stringify(data[i].vlan_id_all) != '{}' ){
                var vidstr='';
                for( var vid in data[i].vlan_id_all ){
                    if( element['netdata']['vlan_list'] && element['netdata']['vlan_list']['data'][vid] ) {
                        vidstr += element['netdata']['vlan_list']['data'][vid]['vlan_id']+"/"
                    }
                }
    ...
    switch_mirror:function(){
        ...
        if( element.netdata['mirror'] && element.netdata['mirror']['data' ] ){
            ...
            for( var i in element.netdata['mirror']['data'] ){
                ...
                if( monitor_port_str ){
                    monitor_port_str = monitor_port_str.substr(0,monitor_port_str.length-1)
                }
                if( mirror_port_str ){
                    mirror_port_str = mirror_port_str
                }
                str += '<tr>'
    ...
    route_interface:function(){
        ...
        if( element.netdata['interface_list'] && element.netdata['interface_list']['data'] ){
            ...
            Ajax$.jsonp({
                url:window_topo_web_path+'/topo_manage/topo_manage/produce_router_port_info?callback=?',
                data:data,
                ok:function(re_data){
                    if( re_data &&  re_data.retCode != undefined && re_data.retCode == 0 && re_data.eth_list ){
    ...
    firewall_safe:function(){
        ...
        if( element.netdata['firewall_safe'] && element.netdata['firewall_safe']['data' ] ){
            ...
            for( var i in element.netdata['firewall_safe']['data'] ){
                ...
                str += '<td>'+element.netdata['firewall_safe']['data'][i].source_port_end+'</td>'
    ...
    firewall_service:function(){
        ...
        if( element.netdata['firewall_service'] && element.netdata['firewall_service']['data' ] ){
            ...
            for( var i in element.netdata['firewall_service']['data'] ){
                ...
                str += '<td>'+element.netdata['firewall_service']['data'][i].port_1+'</td>'
                str += '<td>'+element.netdata['firewall_service']['data'][i].port_2+'</td> '
                str += '</tr>'
            }
            table_obj.append( str );
        }
    ...
    sdn_interface:function(){
        ...
        if( element.netdata['interface_list'] && element.netdata['interface_list']['data'] ){
            ...
            Ajax$.jsonp({
                ...
                data:data,
                ok:function(re_data){
                    if( re_data && re_data.retCode == 0 && re_data.eth_list ){
                        var data = re_data.eth_list;
                        var element = current_element();
                        table_obj.find('tbody tr').not(":first").remove();
                        for(var i=0;i<data.length;i++) {
...
// 2层 3层切换到接口配置
$("#modal9_3_inter,#modal2_2_inter").click(function(){
    restore.switch_24_optical();
})
...
function keep_topo( ){
    ...
    var s = {
        ...
        topo_type:$("#current_topo_type").val()
    };
    ...
    for( var net in link_obj.links ){
        var net_1='';
        var net_2='';
        if( link_obj.links[net].link_net1 ){
            net_1 = link_obj.links[net].link_net1;
        }
        if( link_obj.links[net].link_net2 ){
            net_2 = link_obj.links[net].link_net2;
        }
        s['links'].push(
            {
                link_net1:net_1,
    ...
    if( !current_id ){
        alert_func.error( '无效ID' , 1200)
        return ;
    }
    lastData.topo_name_id = current_id;
    lastData.topo_name = $("#current_topo_name").val();
    lastData.topo_type = $("#current_topo_type").val();
    lastData.web_info = JSON.stringify(s);

    alert_func.loading()
    Ajax$.post({
        url:window_topo_web_path+'/topo_manage/topo_manage/save_topo',
        data:lastData,
        ok:function(msg){
