...
function showJTopoToobar(stage){
    ...
    +'<div id="section-top-bar">'
        ...
        +'</div>'
        +'<div class="icon" id="catch" style="display: none">'
        +'<span class="glyphicon glyphicon-picture" style="font-size: 22px;display: none;transform:  translate(34px,-25px);"></span>抓包分析</span>'
        +'</div>'
        +'</div>'
    ...
    //+ '&nbsp;&nbsp;<input type="button" id="printButton" value="导出PDF">'
        +'<input type="checkbox" id="chart" style="display: none"/><label for="chart" id="chart-label" style="transform:  translate(-483px,11px)"><span> <span  class="glyphicon glyphicon-signal" style="font-size: 22px;display: inline-block;transform:  translate(30px,-26px)"></span>统计分析</span></label>'
    );
    ...
    $('#chart').click(function () {
        ...
        $('#section_chart').show();
        //$('html').append(ala_chart);
        var alalert=$('#chart-label').offset();
        $('#section_chart').css({
            top: alalert.top,
            left:alalert.left-500,
        })
    });
    $('#ala_close').click(function(){
        $('#section_chart').hide();
    })
